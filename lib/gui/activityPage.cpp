...
void createActivityPage(){
    ...
    // Invalidate alle pages om ze opnieuw te renderen
    lv_obj_invalidate(page1.dataPage);
    lv_obj_invalidate(page2.dataPage);
    lv_obj_invalidate(page3.dataPage);
    ...
    lv_obj_add_event_cb(leftButton, leftButtonEventHandler, LV_EVENT_CLICKED, NULL);
     
    lv_obj_t *rightButton = lv_button_create(activityPage);
    lv_obj_set_size(rightButton, 50, 50);
    lv_obj_set_pos(rightButton, 260, 215);
    lv_obj_remove_flag(rightButton, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_event_cb(rightButton, rightButtonEventHandler, LV_EVENT_CLICKED, NULL);

    page1.setWidgetTypes(ACTIVITY_TIMER, ACTIVITY_HEART_RATE);
